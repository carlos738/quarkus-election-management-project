...
public class CandidateApiTest {
    ...
    void update(){
        ...
        assertEquals(api.dto.Candidate.fromDomain(candidate),response);
    }
    @Test
    void list(){
        var candidates = Instancio.stream(Candidate.class).limit(10).toList();

        Mockito.when(candidateService.findAll()).thenReturn(candidates);
        ...
        assertEquals(candidates.stream().map(api.dto.Candidate::fromDomain).toList(), response);
    }
}
